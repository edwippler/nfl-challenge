...
router.get('/averageSinceBye/:team', (req, res) => {
  ...
  pool.query(queries.getTeamByeWeekQuery, [teamAlias])
    .then((result) => {
      ...
      let scoreQueryText = `SELECT AVG(${selectedPeriod}) AS average FROM team_score WHERE team_abbr = $1 AND week > $2`
      pool.query(scoreQueryText, [teamAlias, byeWeek])
        ...
        .catch((error) => {
          ...
          res.send(`Unable to find data for ${teamAlias} in period ${inputPeriod}.\r\nPlease verify team and period provided.`)
        })
    })
    .catch((error) => {
